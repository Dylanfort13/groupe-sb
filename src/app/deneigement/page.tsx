...
export const metadata: Metadata = {
  ...
  description:
    "Déneigement résidentiel, commercial et industriel à Chibougamau, Chapais et environs. Transport de neige, aménagement et vente de matériel granulaire.",
};
...
export default async function DeneigementPage({
  searchParams,
}: {
  ...
  const page = cms.divisionPages["deneigement"];

  return (
    <>
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <Image src={imgSrc(page.hero.backgroundImage, "/deneigement-sb.jpg")} alt="" fill sizes="100vw" className="object-cover grayscale" priority />
        </div>
              ...
              {page.services.cards.map((svc, i) => (
                <RevealOnScroll key={svc.title} delay={i * 100} className="h-full">
                  <div className="group relative bg-charcoal rounded-sm overflow-hidden transition-all duration-700 hover:-translate-y-1.5 hover:shadow-[0_20px_50px_rgba(0,0,0,0.55)] flex flex-col h-full">
                    ...
                    <span className="absolute bottom-0 left-0 w-full sm:w-0 h-[3px] bg-deneigement transition-all duration-700 sm:group-hover:w-full z-[3]" />
                    <div className="relative z-[2] p-8 flex flex-col h-full">
                      <div className="font-display text-xl tracking-[0.04em] text-white mb-4 pb-3 border-b border-deneigement/25 flex items-center gap-2.5">
                        <span className="text-deneigement">{services[i]?.icon}</span>
                        {svc.title}
                      </div>
                      ...
                      <div className="mt-6 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                        <Link
                          href={`/deneigement/${svc.slug}`}
                          className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.1em] uppercase text-deneigement border border-deneigement/40 hover:border-deneigement hover:bg-deneigement/10 px-4 py-2 rounded-sm transition-all duration-200"
                        >
                          En savoir plus
        ...
        <section className="bg-light-bg py-24 px-[5%]">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {[
                { title: "Saison de déneigement", text: "De la première neige jusqu'au 15 avril, 24h sur 24 7j sur 7," },
        ...
        <section className="bg-black-2 py-24 px-[5%]">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-end gap-8 flex-wrap mb-12">
              ...
              <Link href="/realisations" className="text-xs font-bold tracking-[0.1em] uppercase text-deneigement hover:text-white transition-colors">
                Voir tout →
              </Link>
            </div>
